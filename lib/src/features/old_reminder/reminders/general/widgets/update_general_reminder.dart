...
class UpdateGeneralReminder extends ConsumerStatefulWidget {

  final GeneralModel generalModel;

  UpdateGeneralReminder({required this.generalModel,super.key});

  @override
  ConsumerState<UpdateGeneralReminder> createState() => _UpdateGeneralReminderState();
}

class _UpdateGeneralReminderState extends ConsumerState<UpdateGeneralReminder> {



  late GeneralModel general;

  late bool _isReminder;


  late bool _hasRemindBefore;
  ...
  void initState(){
    super.initState();
    general = widget.generalModel;
    reminderId = general.reminderId;
    _isReminder = general.isReminder;
    _titleController.text = general.title;
    _hasRemindBefore = general.hasRemindBefore ?? false;
    if(general.notes != null){
      _noteController.text = general.notes!;
    }
    if(_isReminder){
      selectedTime = general.startDate!;
      selectedPattern = general.reminderPattern!;
      _dateController.text = DateFormat('HH:mm a, MMMM dd, yyyy').format(selectedTime!);
      if(_hasRemindBefore){
        selectedRemindMe = RemindBefore(remindTypeId: general.remindBefore!.remindTypeId, remindType: general.remindBefore!.remindType);
        _beforeController.text = general.remindBefore!.remindMeBefore!.toString();
      }
      if(selectedPattern!.patternId == 3){
        selectedDays = selectedPattern!.daysOfWeek!;
      }
      if(selectedPattern!.patternId == 4){
        intervalTime = selectedPattern!.daysOfInterval!;
      }
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(onPressed: (){
                showGenMenu(context: context, ref: ref, generalModel: general, isFromDetail: true);
              }, icon: Icon(Icons.more_vert,color: MyColors.secondary,))
            ],
          ),
          body: Padding(
  ...
  void _setReminder() async {
    ...
    await AwesomeNotifications().cancel(reminderId*100);
    await Workmanager().cancelByUniqueName('2-${reminderId}');
    ...
    Future.delayed(Duration(seconds: 2),(){
      ref.read(generalReminderNotifier.notifier).editReminder(generalModel: generalModel, context: context).then((value) {
        ref.invalidate(imageListProvider);
