...
class PatientDocumentPage extends ConsumerWidget {
  PatientDocumentPage({super.key});
  ...
  Widget build(BuildContext context,ref) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('Documents',style: wh3,),
          centerTitle: true,
          ...
          },icon: Icon(Icons.chevron_left,color: MyColors.primary,),),
          actions: [
            if(!searchEnabled)
        ...
        body: ConnectivityWidgetWrapper(
          stacked: false,
          offlineWidget: _localFiles(context, ref),
            child: allDocuments.when(
                data: (data){
                  if(data.isEmpty){
                    return RefreshIndicator(
                      ...
                      child: NoFileWidget(text: 'No Files', type: 1),
                    );
                  }
                  ...
                  final filteredFiles = files.where((element) => element.documentTitle.toLowerCase().contains(query)||element.folderName.toLowerCase().contains(query) ||element.completedDate.toString().toLowerCase().contains(query) || element.patientAttachment.toLowerCase().contains(query)).toList();

                  bool haveImage = data.any((element) => element.documentTypeID == 2);
                  bool havePdf = data.any((element) => element.documentTypeID == 1);

                  List<String> folderNames = [];
                  List<String> docNames = data.map((e) => e.documentTitle.toLowerCase()).toList();

                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    ref.read(commonProvider.notifier).updateDocumentTitles(docNames);
                  });
                            ...
                            Visibility(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  h30,
                                  Text('My Folders',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: MyColors.darkGrey),),
                                  h10,
                                  GridView.builder(
                                      itemCount: folderNames.length,
                                      physics: NeverScrollableScrollPhysics(),
                                      shrinkWrap: true,
                                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3,childAspectRatio: 5/4.2,crossAxisSpacing: 8,mainAxisSpacing: 8),
                                      itemBuilder: (context,index){
                                        final numOfFiles = filteredFiles.where((element) => element.folderName == folderNames[index]).toList().length;
                                        return InkWell(
                                          onTap: () {
                                            ref.read(commonProvider.notifier).updateDocumentTypeId(0);
                                            _searchController.clear();
                                            ...
                                            Navigator.push(
                                                context,
                                                routeTo(context,
                                                    PatientFolderPage(title: folderNames[index])));
                                          },
                                          child: Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                Icon(Icons.folder,color: MyColors.primary,size: 30,),
                                                h06,
                                                Text(folderNames[index],style: TextStyle(fontWeight: FontWeight.w500),),
                                                Text('$numOfFiles files',style: TextStyle(fontSize: 10),),
                                              ],
                                            ),
                                          ),
                                        );
                                  ...
                                  Container(
                                    decoration: BoxDecoration(
                                        borderRadius: BorderRadius.circular(10),
                                        color: MyColors.lightGrey.withOpacity(0.6)
                                    ),
                                    padding: allPadding,
                                    child: Column(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  GridView.builder(
                                      ...
                                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3,childAspectRatio: 5/4.2,crossAxisSpacing: 8,mainAxisSpacing: 8),
                                      itemBuilder: (context,index){
                                        final numOfFiles = files.where((element) => element.folderName == folderNames[index]).toList().length;
                                        return InkWell(
                                          onTap: () {
                                            ref.read(commonProvider.notifier).updateDocumentTypeId(0);
                                            ref.read(commonProvider.notifier).enableDocumentSearch(false);
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      ...
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text('Recent Files',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: MyColors.darkGrey),),
                                        h10,
                                        ListView.builder(
                                          // padding: EdgeInsets.zero,
                                          shrinkWrap: true,
  ...
  void _fileDetail(BuildContext context,ref, PatientDocumentModel file) async {
    ...
    final size = (double.parse(data.headers['content-length']?.toList().first ?? '0.0')  * 0.001).toStringAsFixed(1);
      ...
      return AlertDialog(
        ...
        actions: [
          ...
          regularTextButton(text: 'Delete', onPressed: (){
            ref.read(patientDocumentNotifier.notifier).delFile(file,token: user.token!).then((value){
                ref.refresh(patientDocumentProvider(Tuple2(user.userName!, user.token!)));
                Navigator.pop(context);
             });
          })
  ...
  ///offline local data....
  Widget _localFiles(context,ref){
    final user = Hive.box<UserModel>('users').values.first;
    List<PatientDocumentModel> data = ref.watch(patientLocalDocumentProvider(Tuple2(user.userName!, user.token!)));
    final documentTypeId = ref.watch(commonProvider).selectedDocumentType;
    final searchEnabled = ref.watch(commonProvider).documentSearch;
    final query = ref.watch(commonProvider).filteredText.toLowerCase();
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Local',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: MyColors.darkGrey),),
                            h10,
                            ListView.builder(
                              // padding: EdgeInsets.zero,
                              shrinkWrap: true,
                                ...
                                return Column(
                                  children: [
                                    ...
                                    ListTile(
                                      onTap: () async {
                                        if(filteredFiles[index].documentTypeID == 2){
                                          final imgFile = File(filteredFiles[index].attachmentsData!);
                                          final image = Image.file(imgFile).image;
                                          showImageViewer(context, image, onViewerDismissed: () {
                                          });
                                        }
                                        else{
                                          final file = File(filteredFiles[index].attachmentsData!);
                                          Navigator.push(context, routeTo(context, PDFViewerPage(file: file,title:files[index].documentTitle)));
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Local Files',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: MyColors.darkGrey),),
                            h10,
                            ListView.builder(
                              // padding: EdgeInsets.zero,
                              shrinkWrap: true,
                                ...
                                return Column(
                                  children: [
                                    ...
                                    ListTile(
                                      onTap: () async {
                                        if(files[index].documentTypeID == 2){
                                          final imgFile = File(files[index].attachmentsData!);
                                          final image = Image.file(imgFile).image;
                                          showImageViewer(context, image, onViewerDismissed: () {
                                          });
                                        }
                                        else{
                                          final file = File(files[index].attachmentsData!);
                                          Navigator.push(context, routeTo(context, PDFViewerPage(file: file,title:files[index].documentTitle)));
                                      ...
                                      trailing: IconButton(
                                          onPressed: (){
                                            _fileLocalMenu(context, ref, files[index]);
                                          }, icon: Icon(Icons.more_vert,color: MyColors.black,size: 18,)),
                                    ),
  ...
  void _fileLocalMenu(BuildContext context,ref, PatientDocumentModel file) async {
    final icon = file.documentTypeID == 1 ? Icons.picture_as_pdf : Icons.image;
    final fileLink = '${Api.baseUrl}/${file.patientAttachment}';
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  onTap: () async {
                    if(file.documentTypeID == 2){
                      final imgFile = File(file.attachmentsData!);
                      final image = Image.file(imgFile).image;
                      showImageViewer(context, image, onViewerDismissed: () {
                      });
                    }
                    else{
                      final tempFile = File(file.attachmentsData!);
                      Navigator.push(context, routeTo(context, PDFViewerPage(file: tempFile,title:file.documentTitle)));
                ...
                ListTile(
                  onTap: (){
                    Navigator.pop(context);
                    _delLocalFile(context, ref, file);
                  },
                  leading: Icon(Icons.delete_forever_rounded),
                  title: Text('Delete'),
                ),
  ...
  void _delLocalFile(context,ref,PatientDocumentModel file) async{
    final user = Hive.box<UserModel>('users').values.first;
    await showDialog(context: context, builder: (context){
      return AlertDialog(
        title: Text('Confirm Delete'),
        ...
        actions: [
          regularTextButton(text: 'Cancel', onPressed: ()=>Navigator.pop(context)),
          regularTextButton(text: 'Delete', onPressed: (){
            ref.read(patientDocumentNotifier.notifier).delLocalFile(file).then((value){
              ref.refresh(patientLocalDocumentProvider(Tuple2(user.userName!, user.token!)));
              Navigator.pop(context);
            });
          })
